...
const auditoriumsByTypeRegex =
	/\/api\/auditoriumtypes\/[a-zA-Z0-9а-яА-Я\s-]+\/auditoriums/;
...
module.exports = async (req, res, db) => {
	...
	if (pulpitsByFacultyRegex.test(endpoint)) {
		...
		try {
			const response = await db.executeQueryByEndpoint(
				"GET: /api/faculty/pulpits",
				facultyName
			);
	...
	if (auditoriumsByTypeRegex.test(endpoint)) {
		...
		try {
			const response = await db.executeQueryByEndpoint(
				"GET: /api/auditoriumtype/auditoriums",
				auditoriumTypeName
			);
